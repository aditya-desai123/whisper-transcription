...
from main import Transcription
import docx
...
with st.sidebar.form("input_form"):
    ...
    pauses = st.checkbox("Pause The Transcription", value=False)

    translation = st.checkbox("English Transcription", value=False)
...
if "transcription" in st.session_state:
    ...
    for i, output in enumerate(st.session_state.transcription.output):
        ...
        with st.expander("Transcript"):
            color_coding = st.checkbox(
                "Color Coding", value=False, key={i}, help='')
            for idx, segment in enumerate(output['segments']):
                for w in output['segments'][idx]['words']:
    ...
    st.markdown("<h1>UnionPT - Notes Transcription</h1>",
                unsafe_allow_html=True)